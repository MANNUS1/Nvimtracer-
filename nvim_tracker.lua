...
-- Store original content when buffer is read or modified externally
local baselines = {}
...
local function get_project_root(buf_path)
    ...
    while current_dir ~= "" and current_dir ~= "/" do
        if vim.fn.isdirectory(current_dir .. "/.agents") == 1 then
            return current_dir
        end
...
local function generate_delta(bufnr)
    ...
    if buf_path == "" or not vim.fn.filereadable(buf_path) then return end
    ...
    local diff_cmd = string.format("diff -u %s %s", tmp_old, vim.fn.shellescape(buf_path))
